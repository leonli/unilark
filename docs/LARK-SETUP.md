...
已验收普通消息、真实允许/停止、待审批时重启和桌面输入同步；既有私聊无需重跑；0.0.5 独立群需补下方增量配置。
...
本机 2026-09-13 实测：建群、更新邀请/分享权限成功；成员列表返回 99991672，明确缺少成员读取授权。
租户已授权清单没有全量群消息权限。`application:bot.menu:write` 已授权，但现有公开 SDK/文档未找到可核验的私聊菜单写入接口；因此使用官方后台配置及发布流程。
